...
const AppLayout = ({ children }: AppLayoutProps) => {
  ...
  return (
    <div className="flex flex-col md:flex-row min-h-screen bg-[#fafafa]">
      {/* Sidebar for desktop */}
      <div className="hidden md:flex md:w-64 bg-white border-l shadow-sm py-6 flex-shrink-0">
          ...
          <nav className="space-y-1 px-3">
            {navItems.map((item) => (
              <Link
                key={item.path}
                to={item.path}
                className={`flex items-center justify-end px-3 py-2 rounded-md text-sm transition-colors ${
